...
def get_bit_length(sender, app_data, user_data):
    ...
    if dpg.get_value(user_data[f"{s}_input_text"]) != "":
        dpg.set_value(user_data[f"{s}_pow"], value=int(dpg.get_value(user_data[f"{s}_input_text"]), 16).bit_length())
    else:
        dpg.set_value(user_data[f"{s}_pow"], value="")

def drag_pow_callback(sender, app_data, user_data):
    s = (dpg.get_item_label(sender).split("##")[1])
    dpg.set_value(user_data[f"{s}_x_input_text"], value=hex(2 ** dpg.get_value(sender))[2:])

def base_point_callback(sender, app_data, user_data):
    s = (dpg.get_item_label(sender).split("##")[1])
    _, public_key = ecdsa.make_keypair(1)
    _, neg_y = (ecdsa.point_neg(public_key))
    dpg.set_value(user_data[f"{s}_x_input_text"], value=hex(public_key[0])[2:].zfill(64))
    dpg.set_value(user_data[f"{s}_y_input_text"], value=hex(public_key[1])[2:].zfill(64))
    dpg.set_value(user_data[f"{s}_neg_y_input_text"], value=hex(neg_y)[2:].zfill(64))
...
def get_y_coordinate(sender, app_data, user_data):
    ...
    if ecdsa.is_on_curve((x,y)):
        dpg.set_value(user_data[f"{s}_y_input_text"], value=hex(y)[2:].zfill(64))
        dpg.set_value(user_data[f"{s}_neg_y_input_text"], value=hex(neg_y)[2:].zfill(64))
    else:
...
def send_to_memory(sender, app_data, user_data):
    ...
    with dpg.table_row(parent=user_data["memory"]):
        with dpg.group(horizontal=False):
            ...
            # dpg.add_table_next_column(parent=user_data["memory"])
            dpg.add_text(y_value)
            # dpg.add_table_next_column()
            # dpg.add_table_column()

        with dpg.group(horizontal=True):
            dpg.add_button(label="->A", user_data=user_data, callback=recall_memory)
            # dpg.add_same_line(parent=user_data["memory"])
            dpg.add_button(label="->B", user_data=user_data, callback=recall_memory)
            # dpg.add_same_line(parent=user_data["memory"])
            dpg.add_button(label="X", width=24, user_data=user_data, callback=delete_slot_memory)
        # dpg.add_table_column()
...
def select_type(sender, app_data, user_data):
    ...
    s = (dpg.get_item_label(sender).split("##")[1])
    toggle_items = [user_data[f"{s}_base_button"], user_data[f"{s}_y_neg_y_button"], user_data[f"{s}_y_input_text"], user_data[f"{s}_neg_y_input_text"]]
...
def rand_callback(sender, app_data, user_data):
    ...
    if dpg.get_value(user_data[f"{s}_radio_button"]) == "Vector": #vector
        ...
        dpg.set_value(user_data[f"{s}_x_input_text"], value=hex(private_key)[2:].zfill(64))
        batch_set_value([user_data[f"{s}_y_input_text"], user_data[f"{s}_neg_y_input_text"]], "", "")
...
def add_callback(sender, app_data, user_data):
    ...
    if sel_type_a == "Scalar" and sel_type_b == "Scalar": #a scalar, b scalar
        ...
        if A_x != "" and B_x != "":
            ...
            dpg.set_value(user_data["C_x_input_text"], value=hex(result)[2:].zfill(64))
            dpg.configure_item(user_data["C_y_input_text"], enabled=False)
            dpg.configure_item(user_data["C_neg_y_input_text"], enabled=False)
            batch_set_value([user_data["C_y_input_text"], user_data["C_neg_y_input_text"]], "", "")

            _, public_key = ecdsa.make_keypair(result)
            _, neg_y = (ecdsa.point_neg(public_key))
            dpg.set_value(user_data["public_x_input_text"], value=hex(public_key[0])[2:].zfill(64))
            dpg.set_value(user_data["public_y_input_text"], value=hex(public_key[1])[2:].zfill(64))
            dpg.set_value(user_data["public_neg_y_input_text"], value=hex(neg_y)[2:].zfill(64))
        else:
        ...
        if A_x != "" and A_y != "" and B_x != "" and B_y != "":
            ...
            batch_set_value([user_data["public_x_input_text"], user_data["public_y_input_text"], user_data["public_neg_y_input_text"]], "", "")
            ...
            if ecdsa.is_on_curve(point_a)== False or ecdsa.is_on_curve(point_b) == False:
                ...
                if result == None:
                    batch_set_value([user_data["C_x_input_text"], user_data["C_y_input_text"], user_data["C_neg_y_input_text"]], "", 0)
                else:
                    _, neg_y = ecdsa.point_neg(result)
                    dpg.set_value(user_data["C_x_input_text"], value=hex(result[0])[2:].zfill(64))
                    dpg.set_value(user_data["C_y_input_text"], value=hex(result[1])[2:].zfill(64))
                    dpg.set_value(user_data["C_neg_y_input_text"], value=hex(neg_y)[2:].zfill(64))
        else:
            # logger.log_warning("please input point A, B value.")
            pass
    else:
        batch_set_value([user_data["C_x_input_text"], user_data["C_y_input_text"], user_data["C_neg_y_input_text"]], "", "")
...
def mul_callback(sender, app_data, user_data):
    ...
    if (sel_type_a == "Scalar" and sel_type_b == "Scalar"): #a scalar, b scalar
        ...
        if ecdsa.is_on_curve(point_b)== False:
            ...
            result = ecdsa.scalar_multiply(int(A_x, 16), (int(B_x, 16), int(B_y, 16)), 0)
            _, neg_y = (ecdsa.point_neg(result))
            dpg.configure_item(user_data["C_y_input_text"], enabled=True)
            dpg.configure_item(user_data["C_neg_y_input_text"], enabled=True)
            batch_set_value([user_data["public_x_input_text"], user_data["public_y_input_text"], user_data["public_neg_y_input_text"]], "", "")
        ...
        if ecdsa.is_on_curve(point_a)== False:
            ...
            result = ecdsa.scalar_multiply(int(B_x, 16), (int(A_x, 16), int(A_y, 16)), 0)
            _, neg_y = (ecdsa.point_neg(result))
            dpg.configure_item(user_data["C_y_input_text"], enabled=True)
            dpg.configure_item(user_data["C_neg_y_input_text"], enabled=True)
            batch_set_value([user_data["public_x_input_text"], user_data["public_y_input_text"], user_data["public_neg_y_input_text"]], "", "")
